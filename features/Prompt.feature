...
Feature: Deep Agent Search and Task Execution
  ...
  @Checkpoint
  Scenario Outline: Verify Checkpoint
    Given I click the check out from the welcome window
    When I search the prompt "<prompt_user_search>" with follow-up query "<follow_up_query>"
    And I should see the status "Completed" for the task
    Then I should see the search results for the default sample task
    ...
    When I search the prompt "<follow_up_query3>"
    When I search the prompt "<restore_query>"
    Then Verify restored by preview and restore
    When I search the prompt "<follow_up_query4>"
    When I search the prompt "<restore_query>"
    Then Verify restored by preview and panel restore
    When I search the prompt "<follow_up_query5>"
    Then Click on Checkpoint restored and Verify restored
    

    Examples:
      | prompt_user_search                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | follow_up_query                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | follow_up_query2                                                                                                                                                                                                                                              | restore_query      | follow_up_query3                                                                                                                                                                                                                         | follow_up_query4                                                                                                                              | follow_up_query5                                                                                                                                                                                                                  |
      | Create a 3-page website with user authentication (sign up, login, logout) and store user data in a database. The Home page should include a header with navigation, a landing section with the title 'Welcome to DevHub Tools', a short description, and a 'Get Started' CTA button. The second page should be a Tools Dashboard that displays a searchable and filterable table of tools with columns: Tool Name, Category, Status, and Last Updated — pulled from the database. The third page should be a Profile page showing the logged-in user's details with a password update form. Add a footer to all pages with About, Terms, and Contact links. | Yes, please go ahead and build the app using DeepAgent’s in-built PostgreSQL database and hosting for better speed and simplicity. For the tool categories in the dropdown filter, include Development, Design, Analytics, Testing, and Productivity. The tool status options should be Active, Inactive, Maintenance, and Beta. You can pre-populate the database with a few sample tools—around 4 to 5—covering different categories and statuses for demonstration purposes. On the user profile page, please display the user's full name, email, role, and join date along with the password field for login. Let me know if anything else is needed to get started. | Change the headline text on the Home page from 'Welcome to DevHub Tools' to 'Empowering Your Digital Journey' and replace the paragraph with 'Join thousands of developers using our platform to bring their ideas to life — faster, smarter, and with ease.' | restore my changes | Add a new column called 'rating' to the tools table. This column should store a numerical value between 1 and 5 representing the user rating for each tool. Display this new rating value on the Tool List page next to each tool entry. | Remove the 'status' column from the tools table. Make sure to also remove any UI elements like filters or tags that rely on the status value. | Update the entire website's theme colors: set the background to #f5f9ff, headers and primary buttons to #2b6cb0, font color to #1a202c, and hover color for buttons to #2c5282. Apply these colors across all pages consistently. |
